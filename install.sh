...
# True if this skill directory was generated by ContextSect.
#
# Current installs carry an explicit marker. Installs predating the marker do
# not, which would orphan them forever — so fall back to matching the rule's
# own H1 heading, since the old installer copied the rule file verbatim into
# the skill body. Requiring both a known rule name and a matching heading keeps
# an unrelated user skill that merely shares a name safe.
is_managed_skill() {
  local dir="$1" name="$2" h1
  [[ -f "${dir}/SKILL.md" ]] || return 1
  grep -qF "$CS_MARKER" "${dir}/SKILL.md" 2>/dev/null && return 0
  [[ -f "${RULES_DIR}/${name}.md" ]] || return 1
  h1="$(head -1 "${RULES_DIR}/${name}.md")"
  [[ -n "$h1" ]] || return 1
...
prune_stale_skills() {
  ...
  for d in "${dir}"/*; do
    [[ -d "$d" ]] || continue
    base="$(basename "$d")"
    is_managed_skill "$d" "$base" || continue
    if [[ ! -f "${RULES_DIR}/${base}.md" ]]; then
      rm -rf "$d"
